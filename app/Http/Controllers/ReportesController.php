...
class ReportesController extends Controller
{
    ...
    public function savePDF(Request $request)
    {
        ...
        foreach ($productsData as $key => $value) {
            ...
            $result  = round(($amounts['price'] / $cost->dollar * $cost->sale) + $cost->transfer + $cost->amount + $cost->transport + $prices->logo, 2);
            $utility = round($result * $prices->utility, 2); //guardar bd
            $total   = round($result + $utility, 2);
            //IVA $iva = round($total * $cost->iva, 2);
